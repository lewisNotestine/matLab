...
for (i=1:trials)
    ...
    t = timer('StartFcn','WaveRecorder()','TimerFcn','PlaySplat()','StartDelay', 1,...
        'StopFcn','wavwrite(c,Fs,testVis2(i,:))');   % saves auditory information to .wav file.)
    ...
    % records response to auditory stimulus    
    %c = wavrecord(5*Fs,Fs);           
    %wavwrite(c,Fs,testVis2(i,:));   % saves auditory information to .wav file.
